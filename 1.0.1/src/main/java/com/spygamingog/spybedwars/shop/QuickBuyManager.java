...
public class QuickBuyManager {
    ...
    private Map<Integer, String> getDefaultQuickBuy() {
        ...
        if (config.contains("quick-buy-defaults")) {
            for (String key : config.getConfigurationSection("quick-buy-defaults").getKeys(false)) {
                try {
                    int slot = Integer.parseInt(key);
                    defaults.put(slot, config.getString("quick-buy-defaults." + key));
