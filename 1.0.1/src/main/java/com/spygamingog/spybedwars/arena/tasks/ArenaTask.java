...
public class ArenaTask extends BukkitRunnable {
    ...
    private void handleHealPool() {
        for (Team team : arena.getTeams()) {
            boolean hasHealPool = team.getUpgrades().getOrDefault("heal_pool", 0) > 0;
            for (UUID uuid : team.getMembers()) {
                Player p = Bukkit.getPlayer(uuid);
                if (p != null && p.getGameMode() != org.bukkit.GameMode.SPECTATOR) {
                    // Check if player is near their spawn (island)
                    boolean nearSpawn = team.getSpawnPoint() != null && 
                                       p.getLocation().getWorld().equals(team.getSpawnPoint().getWorld()) && 
                                       p.getLocation().distanceSquared(team.getSpawnPoint()) < 900; // 30 block radius
                    
                    if (hasHealPool && nearSpawn) {
                        ...
                        // If near spawn but NO heal pool, disable natural regeneration
                        // BedWars players shouldn't regenerate naturally anyway, but let's be explicit
                        p.setSaturation(0);
                        p.setExhaustion(0);
                    }
                    
                    // Force disable natural health regen for all players in the arena
                    // This is usually handled by game rules, but we can ensure it here
                    if (p.getHealth() < p.getMaxHealth() && !p.hasPotionEffect(org.bukkit.potion.PotionEffectType.REGENERATION)) {
                        p.setHealth(p.getHealth()); // Reset health to current to stop any pending natural regen
                    }
                }
            }
        }
    ...
    private void stopGame() {
        ...
        // Clear all persistent entities (Golems, Fireballs, etc.)
        arena.clearPersistentEntities();
